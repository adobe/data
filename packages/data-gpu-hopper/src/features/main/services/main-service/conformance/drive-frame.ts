...
// skipping the scheduler's own bookkeeping system. The documented headless-host
// pattern (see systems.md), used by the tick-loop and outcome-selection tests.
// Test-only.
export const driveFrame = (db: SystemDatabase): void => {
  for (const tier of db.system.order) {
    for (const name of tier) {
      ...
      // database's declared system keys — an invariant the `string` element type
      // of `order` cannot carry.
