...
/**
 * physicsRenderBridge — makes rigid bodies renderable by `pbrRender`. Once the
 * shape geometries exist, each body gains a `geometry` ref (sphere / cube by
 ...
 *
 * Also declares `Prop` — a render-only placed geometry with a material (e.g. a
 * static floor) that is not a physics body.
 */
    ...
    systems: {
        physicsBridge: {
            ...
            create: db => () => {
                ...
                if (!shapes) return;
                for (const arch of db.store.queryArchetypes(["bodyType", "colliderShape", "halfExtents"], { exclude: ["geometry"] })) {
