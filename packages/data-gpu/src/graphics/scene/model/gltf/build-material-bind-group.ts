...
import { schema as visibleMaterialSchema } from "../../../rendering/visible-material/schema.js";
import type { VisibleMaterial } from "../../../rendering/visible-material/visible-material.js";
import type { GltfAsset, GltfMaterial } from "./gltf-schema.js";

const layout = getStructLayout(visibleMaterialSchema);
...
function defaultMaterial(): VisibleMaterial {
    return {
...
function gltfToVisibleMaterial(m: GltfMaterial): VisibleMaterial {
...
export function buildMaterialBindGroup(
    ...
    const material = materialIndex !== undefined && gltf.materials?.[materialIndex]
        ? gltfToVisibleMaterial(gltf.materials[materialIndex])
        : defaultMaterial();
    const gltfMat = materialIndex !== undefined ? gltf.materials?.[materialIndex] : undefined;

    const structBuffer: TypedBuffer<VisibleMaterial> = createStructBuffer(visibleMaterialSchema, layout.size);
