...
export const jointData = Database.Plugin.create({
    components: {
        ...
        jointAxis:     Vec3.schema,   // hinge axis (A-local); unused for fixed/point
        jointMinLimit: F32.schema,    // hinge lower angle (rad); min >= max ⇒ no limit
        jointMaxLimit: F32.schema,
    },
    archetypes: {
        Joint: ["jointType", "jointBodyA", "jointBodyB", "jointAnchorA", "jointAnchorB", "jointAxis", "jointMinLimit", "jointMaxLimit"],
    },
});
