...
import type { VoxelShapeName } from "./voxel-shape-definitions.js";

/** Store or transaction surface that exposes the voxel shape name index. */
export interface VoxelShapeByNameLookup {
    resources: {
        _voxelShapeByName: Map<VoxelShapeName, Entity> | null;
    };
}

/** Returns the baked-or-pending mesh entity for `name`, or throws if never seeded. */
export const requireVoxelShape = (db: VoxelShapeByNameLookup, name: VoxelShapeName): Entity => {
