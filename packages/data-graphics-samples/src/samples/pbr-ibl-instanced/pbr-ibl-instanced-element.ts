...
import { createPbrIblInstancedService } from "./pbr-ibl-instanced-service.js";
...
export class PbrIblInstancedElement extends LitElement {
    static styles = css`
        :host { display: block; }
    ...
    private service = createPbrIblInstancedService();
    ...
    override firstUpdated() {
        ...
        for (let x = 0; x < GRID; x++) {
            for (let z = 0; z < GRID; z++) {
                this.service.transactions.insertModel({
        ...
        const unsub = this.service.observe.entity(geoId)((values: unknown) => {
            ...
            const gridExtent = ((GRID - 1) / 2) * SPACING;
            this.service.transactions.setOrbit({
                center: [0, 0, 0],
                radius: gridExtent + size * 2,
                height: size * 0.5,
            });
            this.status = `IBL · ${GRID * GRID} instances`;
        });
    }
