...
// `Pick<Services, "random" | …>`, so the key and its type come from one place and
// can't drift per-transition. The ecs `service-database` registers these same keys
// and is pinned to this map (see its drift-guard).
