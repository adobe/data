...
// so a case can assert "any number" for a value it does not pin. Tic-tac-toe's
// `State` exposes no ecs-minted ids (marks fold into the board string, scores are
// plain counters), so no case needs one today — but the comparison stays
// matcher-aware to match the shared pattern and stay robust if one is ever added.
...
// equal. `+ 0` normalises `-0` to `0`. (Tic-tac-toe's scalars are integer
// counters and the board is a string, so this is a no-op here — kept to mirror
// the shared pattern and stay robust if a float field is added.)
...
// the ecs conformance runners. `after` may use asymmetric matchers, so this one
...
// spec tests and computed conformance, where the compared value is a scalar
