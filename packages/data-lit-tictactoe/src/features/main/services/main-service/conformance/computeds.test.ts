...
// Each `data/state` derivation's cases run against its same-named ecs computed. A
// computed is an `Observe`, so after seeding the store we read its synchronous
// emission and `matches(value)`. This is the computed analog of the
// transaction/action runners.
//
...
// at construction (the agent services do) would `withCache` the pre-seed value,
// and a direct `fromState` seed emits no transaction to invalidate it.
...
// shape a derivation yields. Override for a computed whose output is not a list of
// entities (a scalar, a single entity, a nested shape).
...
// `currentPlayer` emits a scalar `PlayerMark`, so the projection is the identity —
// no entity hydration.
conformsComputed("currentPlayer", {
