...
export class TodoToolbarElement extends TodoElement {
  ...
  render() {
    ...
    const addTodo = () => {
      ...
      if (name === "") return;
      this.service.transactions.createTodo({ name });
      setDraftName("");
    };
    ...
    return presentation.render({
      ...
      setDraftName,
      addTodo,
      addBulkTodos: (count) => this.service.transactions.createBulkTodos({ count }),
      toggleDisplayCompleted: () => this.service.transactions.toggleDisplayCompleted(),
      clearAll: () => this.service.transactions.deleteAllTodos(),
    });
  }
}
