...
import { entity } from "./conformance-case.js";
...
// Spec-owned cases, shared with the ecs `selectTodo` transaction. `args.id` is an
// `entity(specId)` — resolved to the seeded entity on the ecs side. `after` writes
// `selectedTodo` as the PLAIN spec-id of the referenced todo; conformance compares
...
export const cases: Conformance<typeof selectTodo> = [
  {
