...
//
// Serverless WebRTC signaling via copy-paste. Internal to the signaling-service
// implementation (see `../create.ts`); not part of the public contract.
//
// The full ICE candidate list is embedded in the SDP by waiting for
// iceGatheringState === "complete" before exposing the blob — one copy-paste in
// each direction is sufficient for the initial handshake.
//
// Both peers pre-create two `negotiated: true` data channels with matching
// stream ids:
//   - id 0 ("sync")   — carries @adobe/data-sync messages
//   - id 1 ("signal") — carries WebRTC renegotiation messages (ICE restart).
