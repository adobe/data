 ...
 * `renderPresence` callbacks receive a fully-typed `service` argument matching
 * the plugin they passed in. The element internally stores the game database as
 * `unknown` (the negotiation plugin is monomorphic), so we adapt the typed
 * callbacks at this boundary. The cast is justified: the connection service
 * constructs the game database from `args.gamePlugin`, so by construction the
 * value stored in the `gameDb` resource is a `Database.Plugin.ToDatabase<P>`.
 */
