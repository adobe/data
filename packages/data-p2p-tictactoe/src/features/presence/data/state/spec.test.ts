...
// `import.meta.glob`, so a new one is covered the moment it ships. Each
// participating file must export exactly its function plus `cases` (enforced
// below). A case's shape selects the check: `after` → a transition; `value` → a
// derivation.
  ...
  describe(`State.${label}`, () => {
    ...
    for (const testCase of cases) {
      ...
      it(transitionCase.name, async () => {
        ...
        const { args, calls } =
          raw && typeof raw === "object" ? recordArgServices(raw) : { args: raw, calls: {} };
