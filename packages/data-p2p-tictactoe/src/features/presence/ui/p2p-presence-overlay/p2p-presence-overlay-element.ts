...
//
// Optional presence overlay container. Subscribes to peer cursor positions from
// the synced game database and feeds local pointer movement back through the
// `trackPresence` action. A pure container: it observes state, observes local
// pointer events (a genuine UI concern), and delegates everything else to the
// action / presentation.
...
export class P2pPresenceOverlayElement extends DatabaseElement<typeof MainService.plugin> {
    ...
    render() {
        ...
        useEffect(() => {
            ...
            // Normalise local pointer events against the overlay's own size — a DOM
            // concern that has to live in the element — then hand the stream to the
            // action, which owns the transient transaction.
        ...
        // player's mark when the game DB was created); survives the restricted view
        // since it is a plain value.
