...
// database (see `service-database.ts`) and driven through the actions layer
// (see `negotiation-database.ts`), so the UI never touches this surface
// directly — it calls `service.actions.startHost()` and renders purely from
// observable state.
//
