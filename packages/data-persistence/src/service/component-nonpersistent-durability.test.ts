...
const plugin = Database.Plugin.create({
    ...
    archetypes: {
        Item: ["value", "cache", "derived"],
    },
    transactions: {
        spawn(t, args: { value: number; cache: number; derived: number }) {
...
describe("component-level nonPersistent durability", () => {
    ...
    it("does not persist nonPersistent columns: defaulted resets, no-default strips", async () => {
        ...
        const e = db1.transactions.spawn({ value: 5, cache: 99, derived: 42 })!;
        await svc1.flush();
        ...
        const view = db2.read(e) as { value: number; cache?: number; derived?: number } | null;
