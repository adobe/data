...
// Collapse F32↔f64 storage rounding (the ecs `rotation` column is F32, the spec
// authors plain numbers) onto a small grid so float noise compares equal. `+ 0`
// normalises `-0` to `0`.
...
// order-sensitive arrays (`toState` reads sprites in insertion order, matching
// the case's authored order). Exported so it can back other conformance
// comparisons (e.g. computed values).
...
// spec tests and computed conformance, where the compared value is a `Sprite[]`
// or a scalar rather than a whole `State`.
