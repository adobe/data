...
//
// Compile-time type checks for Schema.fromObjectProperties and Schema.fromArchetype.
...
function fromObjectPropertiesWithRequiredTuple() {
  const schema = Schema.fromObjectProperties(
...
function fromObjectPropertiesWithoutRequired() {
  const schema = Schema.fromObjectProperties({ a: Vec3.schema, b: U32.schema });
...
function fromObjectPropertiesToTypeMatchesExactComponentMap() {
  const schema = Schema.fromObjectProperties(
...
function fromObjectPropertiesExtraPropertiesBecomeOptionalInToType() {
  const schema = Schema.fromObjectProperties(components, ["foo", "bar"]);
  type T = Schema.ToType<typeof schema>;
