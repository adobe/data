...
describe("Guid.toUUID", () => {
    ...
    it("encodes a known Guid to the expected string", () => {
        ...
        const g: Guid = [0x12345678, 0x9abcdef0, 0x11223344, 0x55667788];
        expect(Guid.toUUID(g)).toBe("12345678-9abc-def0-1122-334455667788");
    });

    it("handles nil (all zeros)", () => {
        expect(Guid.toUUID(Guid.nil)).toBe("00000000-0000-0000-0000-000000000000");
    });

    it("handles max u32 values", () => {
        const g: Guid = [0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF];
        expect(Guid.toUUID(g)).toBe("ffffffff-ffff-ffff-ffff-ffffffffffff");
    });
